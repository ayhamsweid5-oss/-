...
async function login(username, password) {
  let r = await query('SELECT * FROM users WHERE username=',[username]);
  if (!r.rowCount && username === process.env.ADMIN_USERNAME && password === process.env.ADMIN_PASSWORD) {
    const hash = await bcrypt.hash(password, 12);
    await query('INSERT INTO users(id,username,password_hash,role) VALUES(,,,) ON CONFLICT(username) DO NOTHING',[crypto.randomUUID(),username,hash,'admin']);
    r = await query('SELECT * FROM users WHERE username=',[username]);
  }

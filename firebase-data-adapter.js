...
(() => {
  ...
  async function removeProductCodes() { if(!valid || !user()) throw Error('UNAUTHENTICATED'); const branchId=await branch(), snap=await db.collection('materials').where('branchId','==',branchId).get(); let batch=db.batch(), n=0; for(const d of snap.docs){ const x=d.data(), fields={}; ['sku','productCode'].forEach(k=>{if(Object.prototype.hasOwnProperty.call(x,k)) fields[k]=firebase.firestore.FieldValue.delete();}); if(Object.keys(fields).length){batch.update(d.ref,fields); n++;} if(n%400===0&&n){await batch.commit();batch=db.batch();}} if(n%400) await batch.commit(); return n; }
  /* Product-code generation was removed; materials are identified by documentId. */
